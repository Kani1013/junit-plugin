...
(function(mod) {
  ...
  function collapseSingle(cm, from, to) {
    ...
    function clear() {
      ...
      collapse = false;
      document.getElementById('buttonCollapse').setAttribute("title", "Collapse identical codelines");
      document.getElementById('buttonCollapse').innerHTML = "Collapse";
      FSB_update();
    }
  ...
  function goNearbyDiff(cm, dir) {
    ...
    if (found != null) {
        cm.setCursor(found + (2 * dir), 0); //scroll a little bt over so the merge-button is visible
    	cm.setCursor(found, 0);				//go back to actual line to avoid bugs
    }
    else
      return CodeMirror.Pass;
  }
  
  CodeMirror.commands.existsNextDiff = function(cm, line) {
	  var dv = cm.state.diffViews[0];
	  var isOrig = cm == dv.orig;
	  if (line == null) line = cm.getCursor().line;
	  return findNextDiff(dv.chunks, line, isOrig);
  }
  
  CodeMirror.commands.existsPrevDiff = function(cm, line) {
	  var dv = cm.state.diffViews[0];
	  var isOrig = cm == dv.orig;
	  if (line == null) line = cm.getCursor().line;
	  return findPrevDiff(dv.chunks, line, isOrig);
  }
